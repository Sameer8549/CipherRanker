    ...
    copy.sort((a, b) => {
      ...
      switch (sortKey) {
        ...
        case 'rr':
          ...
          break
        case 'delta': va = a.rankDelta ?? 0; vb = b.rankDelta ?? 0; break
        case 'stability': va = a.stability?.stability ?? 0; vb = b.stability?.stability ?? 0; break
        ...
        <table>
          <thead>
            <tr>
              ...
              <th style={{ cursor: 'pointer' }} onClick={() => handleSort('delta')}>ΔRANK <SortIcon col="delta" /></th>
              <th style={{ cursor: 'pointer' }} onClick={() => handleSort('stability')}>STABLE <SortIcon col="stability" /></th>
              <th>SIGNALS</th>
                  ...
                  <AnimatePresence key={candidate.candidate_id} initial={false}>
                    ...
                    >
                      ...
                      <td>
                        ...
                        <span className="block text-[10px] text-white mt-0.5">{profile.anonymized_name || '—'}</span>
                      </td>
                      ...
                      <td className="font-mono text-xs" style={{ color: (result.rankDelta || 0) > 0 ? '#00D084' : (result.rankDelta || 0) < 0 ? '#FF6B6B' : '#777' }}>
                        {(result.rankDelta || 0) > 0 ? '+' : ''}{result.rankDelta || 0}
                      </td>
                      <td className="font-mono text-xs" style={{ color: (result.stability?.stability || 0) >= 0.9 ? '#00D084' : '#F0B429' }}>
                        {Math.round((result.stability?.stability || 0) * 100)}%
                      </td>
                    ...
                    {isExpanded && (
                      <tr key={`${candidate.candidate_id}-expanded`} style={{ background: 'rgba(204,255,0,0.02)' }}>
                        <td colSpan={12} style={{ padding: '16px 24px', borderBottom: '1px solid #1f1f1f' }}>
                          <motion.div
